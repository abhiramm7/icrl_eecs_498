...
class replay_memory_agent():
    def __init__(self, states_len, replay_window):
        ...
        self.replay_memory = {'states': replay_stacker(self.states_len, self.replay_window),
                              'states_1': replay_stacker(self.states_len,self.replay_window),
                              'states_2': replay_stacker(self.states_len,self.replay_window),
                              'rewards': replay_stacker(1,self.replay_window),
                              'rewards_1': replay_stacker(1,self.replay_window),
                              'actions': replay_stacker(1,self.replay_window),
                              'actions_1': replay_stacker(1,self.replay_window),
                              'terminal': replay_stacker(1,self.replay_window),
                              'terminal_1': replay_stacker(1,self.replay_window)}
    ...
    def replay_memory_update(self,
        ...
        self.replay_memory['rewards_1'].update(rewards_1)
        self.replay_memory['states'].update(states)
        self.replay_memory['states_1'].update(states_1)
        self.replay_memory['states_2'].update(states_2)
        self.replay_memory['actions'].update(actions)
        self.replay_memory['actions_1'].update(actions_1)
        self.replay_memory['terminal'].update(terminal)
        self.replay_memory['terminal_1'].update(terminal_1)
...
class deep_q_agent:
    def __init__(self,
        ...
        self.state_1_vector = np.zeros((1, self.states_len))
        self.state_2_vector = np.zeros((1, self.states_len))

        self.rewards_vector = np.zeros((1))
        self.rewards_1_vector = np.zeros((1))

        self.terminal_vector = np.zeros((1))
        self.terminal_1_vector = np.zeros((1))

        self.action_vector = np.zeros((1))
        self.action_1_vector = np.zeros((1))
 
        self.training_batch = {'states': np.zeros((self.batch_size, self.states_len)),
                               'states_1': np.zeros((self.batch_size, self.states_len)),
                               'states_2': np.zeros((self.batch_size, self.states_len)),
                               'actions': np.zeros((self.batch_size, 1)),
                               'actions_1': np.zeros((self.batch_size, 1)),
                               'rewards': np.zeros((self.batch_size, 1)),
                               'rewards_1': np.zeros((self.batch_size, 1)),
                               'terminal': np.zeros((self.batch_size, 1)),
                               'terminal_1': np.zeros((self.batch_size, 1))}

    def _random_sample(self):
        temp_l= len(self.replay.replay_memory['states'].data())

        indx = randombatch(self.batch_size+2, temp_l) # Increase the width of the sample
    ...
    def _train(self):
        ...
        temp_states_2 = self.training_batch['states_2']
        temp_states_1 = self.training_batch['states_1']
        temp_states = self.training_batch['states']

        temp_rewards = self.training_batch['rewards']
        temp_rewards_1 = self.training_batch['rewards_1']

        temp_terminal = self.training_batch['terminal']
        temp_terminal_1 = self.training_batch['terminal_1']

        temp_actions = self.training_batch['actions']
        temp_actions_1 = self.training_batch['actions_1']

        q_values_train_1 = self.target_model.predict_on_batch(temp_states_1)
        q_values_train_2 = self.target_model.predict_on_batch(temp_states_2)
        ...
        action_space_map = {"00":2, "01":3, "10":4, "11":5}
        ...
        for i in range(self.batch_size):
            ...
            if temp_terminal[i]:
                ...
                target[i][action_idx] = temp_rewards[i] + 0.99 * np.amax(q_values_train_1[i])
            
            action_idx1 = action_space_map[str(action_idx) + str(int(temp_actions_1[i]))]

            if temp_terminal_1[i]:
                target[i][action_idx1] = temp_rewards[i] + 0.99 * temp_rewards_1[i]
            else:
                target[i][action_idx1] = temp_rewards[i] + 0.99 * temp_rewards_1[i] + 0.99 * 0.99 * np.amax(q_values_train_2[i])

            temp_s = np.asarray(temp_states[i])
            s_f[i,:] = temp_s.reshape(1,4)

            temp_t = np.asarray(target[i])
            t_f[i,:] = temp_t.reshape(1,6) # Six action space

        self.ac_model.fit(s_f, t_f, batch_size=32, verbose=0)
